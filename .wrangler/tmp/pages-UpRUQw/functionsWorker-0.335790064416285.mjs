...
// ../.wrangler/tmp/bundle-ZmDSWv/checked-fetch.js
...
async function signJWT(payload, secret) {
  const header = b64url(ENC2.encode(JSON.stringify({ alg: "HS256", typ: "JWT" })));
  const body = b64url(ENC2.encode(JSON.stringify({ ...payload, iat: Math.floor(Date.now() / 1e3) })));
  const key = await getKey(secret);
  const sig = await crypto.subtle.sign("HMAC", key, ENC2.encode(`${header}.${body}`));
...
async function verifyJWT(token, secret) {
  ...
  const key = await getKey(secret);
  const valid = await crypto.subtle.verify("HMAC", key, b64urlDecode(sig), ENC2.encode(`${header}.${body}`));
...
var cors2 = { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json" };
async function onRequestPost2({ request, env }) {
  const { email, password } = await request.json().catch(() => ({}));
  if (!email || !password) return new Response(JSON.stringify({ error: "Email and password required" }), { status: 400, headers: cors2 });
  if (password.length < 8) return new Response(JSON.stringify({ error: "Password must be at least 8 characters" }), { status: 400, headers: cors2 });
  const existing = await env.DB.prepare("SELECT id FROM users WHERE email = ?").bind(email.toLowerCase()).first();
  if (existing) return new Response(JSON.stringify({ error: "Email already in use" }), { status: 409, headers: cors2 });
  ...
  return new Response(JSON.stringify({ token }), { status: 201, headers: cors2 });
}
__name(onRequestPost2, "onRequestPost");
async function onRequestOptions2() {
  return new Response(null, { headers: { ...cors2, "Access-Control-Allow-Methods": "POST", "Access-Control-Allow-Headers": "Content-Type,Authorization" } });
}
...
async function onRequestGet({ request, env }) {
  ...
  if (!user) return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: cors3 });
  const row = await env.DB.prepare("SELECT state_json FROM user_state WHERE user_id = ?").bind(user.sub).first();
  return new Response(row ? row.state_json : "null", { headers: cors3 });
}
...
async function onRequestPut({ request, env }) {
  ...
  return new Response(JSON.stringify({ ok: true }), { headers: cors3 });
}
__name(onRequestPut, "onRequestPut");
async function onRequestOptions3() {
  return new Response(null, { headers: { ...cors3, "Access-Control-Allow-Methods": "GET,PUT" } });
}
__name(onRequestOptions3, "onRequestOptions");

// ../.wrangler/tmp/pages-UpRUQw/functionsRoutes-0.33480512817667085.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-ZmDSWv/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-ZmDSWv/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.335790064416285.mjs.map
